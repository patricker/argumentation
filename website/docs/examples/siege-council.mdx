...
<AttackGraph
  ...
  height={420}
  caption="Five attacks. Aleric (fortify) and Maren (abandon) attack each other — a mutual contested pair. Drust's sortie attacks Maren's retreat (the enemy is thinner than feared, no need to abandon) and Liss's evacuation (no time for a full evacuation). Liss (evacuate) attacks Aleric (fortify): civilians must leave first. Edge weights shown are the *cold-climate* values; warm-climate weights are halved."
/>
